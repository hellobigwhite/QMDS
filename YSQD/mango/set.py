...
# ── 从第一个脚本搬过来的 WP Rocket 设置函数 ── 几乎原封不动
def RocketSetting(site, session):
    # 注意：这里 site 是域名（不带 www），且已经登录成功
    try:
        # 获取插件页面
        wp_url = f'https://www.{site}/wp-admin/plugins.php'
        wp_response = session.get(wp_url, verify=False, timeout=20)
        if wp_response.status_code != 200:
            write_failed(site, "WP_ROCKET", f"无法访问插件页面 状态码 {wp_response.status_code}")
            return False

        soup = BeautifulSoup(wp_response.text, 'html.parser')

        # 尝试激活（如果有激活链接）
        activate_link = soup.find('a', {'id': 'activate-wp-rocket'})
        if activate_link and 'href' in activate_link.attrs:
            activate_url = urljoin(f"https://www.{site}/wp-admin/", activate_link['href'])
            session.get(activate_url, verify=False, timeout=15)
            print(f"[启动] 已尝试激活 WP Rocket: {site}")
        else:
            print(f"[成功] WP Rocket 可能已激活: {site}")

        # 获取设置页面
        setting_url = f'https://www.{site}/wp-admin/options-general.php?page=wprocket'
        st_response = session.get(setting_url, verify=False, timeout=20)
        if st_response.status_code != 200:
            write_failed(site, "WP_ROCKET", f"无法访问设置页面 状态码 {st_response.status_code}")
            return False

        soup = BeautifulSoup(st_response.text, 'html.parser')

        try:
            wpnonce = soup.find('input', {'id': '_wpnonce'})['value']
            secret_key = soup.find('input', {'id': 'secret_key'})['value']
            minify_js_key = soup.find('input', {'id': 'minify_js_key'})['value']
            consumer_email = soup.find('input', {'id': 'consumer_email'})['value']
            consumer_key = soup.find('input', {'id': 'consumer_key'})['value']
            version = soup.find('input', {'id': 'version'})['value']
            minify_css_key = soup.find('input', {'id': 'minify_css_key'})['value']
            wplicense = soup.find('input', {'id': 'license'})['value']
        except Exception as e:
            write_failed(site, "WP_ROCKET", f"无法提取表单隐藏字段 {str(e)}")
            return False
        ...
        resp = session.post(option_url, data=setting_data, verify=False, timeout=40)

        if resp.status_code in (200, 302):
            write_success(site, "WP_ROCKET", "设置完成")
            print(f"------------------[OK] WP Rocket 设置成功: {site} [OK]------------------")
            return True
        else:
            write_failed(site, "WP_ROCKET", f"提交设置失败 状态码 {resp.status_code}")
            return False
...
# ---------- CONFIG ----------
BASE_DIR = r"D:\logo"  # <-- 修改为D:\logo
SUCCESS_LOG = "success.log"
FAILED_LOG = "failed.log"
# 固定密码配置（优先级：命令行 --password > 环境变量 WP_PASSWORD > 固定值）
DEFAULT_PASSWORD = os.environ.get("WP_PASSWORD", "f!XsS$J2WneOkMyUgQ")  # [红色] 你的固定密码
ICON_NAME = "icon.png"
...
def write_success(site, action, extra=""):
    ...
    print(f"[OK] [{site}] {action} 成功 {extra}")
...
def write_failed(site, action, reason=""):
    ...
    print(f"[错误] [{site}] {action} 失败: {reason}")
    ...
    for i in range(retries):
        try:
            ...
            if resp is not None and resp.status_code in (200, 201):
                ...
                print(f"[警告] 状态码 {getattr(resp, 'status_code', None)} 第 {i + 1}/{retries} 次重试: {url}")
        except requests.exceptions.RequestException as e:
            print(f"[警告] 请求异常: {e}，{method} {url}，第 {i + 1}/{retries} 次重试")
        time.sleep(delay)
    return None
...
def resp_to_json(resp):
    ...
    try:
        ...
        print(f"[警告] 响应 JSON 解析失败: {e}")
        return None
    except Exception as e:
        print(f"[警告] 响应解析未知错误: {e}")
        return None
...
def query_existing_icon(session, site, request_fn):
    ...
    try:
        ...
        print(f"[警告] query_existing_icon JSON 解析出错: {e}")
    except Exception as e:
        print(f"[警告] query_existing_icon 未知错误: {e}")
    return None, None
    ...
    for root, dirs, files in os.walk(domain_folder):
        if ICON_NAME in files:
            ...
            if file_size > MAX_ICON_SIZE:
                print(f"[警告] icon文件过大({file_size / 1024 / 1024:.2f}MB)，超过限制: {path}")
                return None
            return path
    return None
...
def upload_icon(session, site, icon_path, upload_nonce):
    ...
    try:
        ...
        print(f"[警告] upload_icon JSON 解析出错: {e}")
    except Exception as e:
        print(f"[警告] upload_icon 未知错误: {e}")
    return None, None
    ...
    if resp and resp.status_code == 200:
        try:
            ...
            if js and js.get("success") and js.get("data", {}).get("id"):
                ...
                print(f"[警告] crop_icon 返回 success=false 或无 id: {site}")
        except Exception as e:
            print(f"[警告] crop_icon JSON 解析出错: {e}")
    else:
        print(f"[警告] crop_icon 请求失败，状态码: {getattr(resp, 'status_code', '无响应')}")
...
def save_wp_settings(session, site, request_fn, site_icon_id=None, date_format=None, time_format=None,
    ...
    if not resp:
        print(f"[警告] 无法获取 options-general.php: {site}")
        return False

    soup = BeautifulSoup(resp.text, "html.parser")
    form = soup.find("form", {"action": "options.php"})
    if not form:
        print(f"[警告] 未找到 options 表单: {site}")
        return False
    ...
    if resp2 and resp2.status_code in (200, 302):
        ...
        print(f"[警告] 保存设置失败，状态码: {getattr(resp2, 'status_code', '无响应')}")
        return False
...
def process_banner(site_folder, session, request_fn):
    ...
    # else:
    #     print(f"[警告] 未找到财神图片: {CAISHEN_PATH}")
...
def get_yoast_nonce(session, site, request_fn, dump_on_fail=True):
    ...
    if not html:
        if dump_on_fail:
            print(f"[错误] 无法获取 Yoast 页面 HTML: {site}")
        return None
    patterns = [
    ...
    if dump_on_fail:
        try:
            ...
            print(f"[搜索] 未找到 nonce，页面已保存至 {fn} 供人工检查")
        except Exception as e:
            print(f"[警告] 写入调试文件失败: {e}")
    return None
    ...
    try:
        ...
        try:
            ...
            if activate_free and activate_free.get('href'):
                ...
                print(f"[启动] 尝试激活 Yoast SEO (Free) : {site}")
            else:
                print(f"[成功] SEO 已激活: {site}")
        except Exception:
            print(f"[成功] SEO 已激活: {site}")
        ...
        try:
            ...
            if activate_premium and activate_premium.get('href'):
                ...
                print(f"[启动] 尝试激活 Yoast SEO Premium: {site}")
            else:
                print(f"[成功] SEO Premium 已激活: {site}")
        except Exception:
            print(f"[成功] SEO Premium 已激活: {site}")
        ...
        if activate_yoast_optimize(session, site, nonce, request_fn):
            print(f"[OK] 已标记 {site} 的 Yoast SEO 数据优化为完成")
        else:
            print(f"[错误] {site} Yoast 数据优化失败")
            all_ok = False

        if set_yoast_site_representation(session, site, nonce, request_fn):
            print(f"[OK] 已设置 {site} 的 Yoast 站点信息")
        else:
            print(f"[错误] {site} Yoast 站点信息设置失败")
            all_ok = False

        if save_yoast_configuration_state(session, site, nonce, request_fn):
            print(f"[OK] 已保存 {site} 的完成步骤")
        else:
            print(f"[错误] {site} 保存完成步骤失败")
            all_ok = False

        if set_yoast_social_profiles(session, site, nonce, request_fn):
            print(f"[OK] 已设置 {site} 的 Yoast 社交媒体信息")
        else:
            print(f"[错误] {site} 设置社交媒体信息失败")
            all_ok = False

        if set_yoast_tracking(session, site, nonce, request_fn):
            print(f"[OK] 已关闭 {site} 的 Yoast 追踪功能")
        else:
            print(f"[错误] {site} 设置 tracking 失败")
            all_ok = False
...
def get_wpseo_page_settings_data(session, site, request_fn):
    ...
    resp = request_fn(session, "GET", url)
    if not resp or resp.status_code != 200:
        print(f"[错误] 无法访问 wpseo_page_settings 页面: {site} (状态码: {getattr(resp, 'status_code', '无响应')})")
        return None
    ...
    if missing:
        print(f"[错误] 关键参数缺失，无法继续高级 Yoast 配置: {missing}")
        return None
...
def process_site(site_folder, args):
    ...
    # ── 新增：WP Rocket 模块 ──
    if (only_set is None) or ("rocket" in only_set):
        try:
            print(f"--- WP ROCKET: {site} ---")
            success = RocketSetting(site, session)
            if not success:
                print(f"[错误] WP Rocket 处理失败: {site}")
        except Exception as e:
            write_failed(site, "WP_ROCKET", str(e))
...
def main():
    ...
    parser.add_argument("--sites", default="", help="要处理的站点，多个站点用逗号分隔")
    args = parser.parse_args()

    base_dir = args.base_dir
    if not os.path.isdir(base_dir):
        print(f"[错误] 无效的 base-dir: {base_dir}")
        sys.exit(1)
    ...
    # 如果指定了站点，则只处理指定的站点
    if args.sites:
        specified_sites = [site.strip() for site in args.sites.split(",")]
        sites_to_process = [os.path.join(base_dir, site) for site in specified_sites if os.path.isdir(os.path.join(base_dir, site))]
        print(f"[循环] 处理指定的 {len(sites_to_process)} 个站点")
    else:
        # 如果失败日志存在，则只处理失败站点
        if os.path.exists(FAILED_LOG):
            ...
            if failed_sites:
                print(f"[循环] 检测到失败日志，重新处理 {len(failed_sites)} 个失败站点")
