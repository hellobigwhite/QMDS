...
def process_icon(site_folder, session, request_fn, date_format="F j, Y", time_format="g:i a", week_starts_on=1):
    ...
    if media_id and crop_nonce:
        ...
        if not icon_path:
            raise RuntimeError("本地未找到 icon.png 或文件过大")
        ...
        final_id = crop_icon(session, site, media_id, crop_nonce) or media_id
    ok = save_wp_settings(session, site, request_fn, site_icon_id=final_id, date_format=date_format,
                          time_format=time_format, week_starts_on=week_starts_on)
    if not ok:
