...
def get_upload_nonce(session, site, request_fn):
    ...
    for attempt in range(3):
        ...
        # 优先找隐藏 input
        input_nonce = soup.find("input", {"name": "_wpnonce"}) or soup.find("input", {"id": "_wpnonce"})
        if input_nonce and input_nonce.get("value"):
            return input_nonce["value"]

        # 找 js 变量
        for script in soup.find_all("script"):
            if not script.string:
                continue
            m = re.search(r'_wpnonce["\']?\s*[:=]\s*["\']([a-zA0-9]{10,})["\']', script.string)
            if m:
                return m.group(1)
        time.sleep(1.2)
    return None
...
def query_existing_icon(session, site, request_fn):
    ...
    js = resp_to_json(resp)  # 使用你第一个脚本里的安全解析函数
...
def upload_icon(session, site, icon_path, upload_nonce):
    ...
    try:
        with open(icon_path, "rb") as f:
            ...
            resp = session.post(upload_url, data=data, files=files, headers=headers, verify=False, timeout=35)
    except Exception as e:
        print(f"上传异常: {e}")
        return None, None
    ...
    if js and js.get("success") and js.get("data", {}).get("id"):
        return js["data"]["id"], js.get("data", {}).get("nonces", {}).get("edit")
    return None, None
...
def crop_icon(session, site, media_id, crop_nonce):
    ...
    crop_data = {
        ...
        "cropDetails[x1]": 0,
        "cropDetails[y1]": 0,
        "cropDetails[x2]": 512,  # 直接用目标尺寸，避免比例问题
        "cropDetails[y2]": 512,
    ...
    headers = {"Referer": f"https://www.{site}/wp-admin/customize.php", "User-Agent": "Mozilla/5.0"}
    resp = session.post(ajax_url, data=crop_data, headers=headers, verify=False, timeout=25)
...
def process_icon(site_folder, session, request_fn, date_format="F j, Y", time_format="g:i a", week_starts_on=1):
    ...
    try:
        media_id, edit_nonce = query_existing_icon(session, site, request_fn)

        if media_id and edit_nonce:
            print(f"  已存在 icon (id={media_id})，直接裁剪")
            final_id = crop_icon(session, site, media_id, edit_nonce) or media_id
        else:
            icon_path = get_last_icon_path(site_folder)
            if not icon_path:
                raise RuntimeError("未找到 icon.png 或文件过大")
        ...
        # 保存设置
        ok = save_wp_settings(session, site, request_fn,
