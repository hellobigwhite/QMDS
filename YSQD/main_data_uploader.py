...
class MainDataUploader:
    ...
    def _login(self, site):
        site = str(site or "").strip().lower().replace("https://", "").replace("http://", "").strip("/")
    ...
    def _get_update_img_url(self, site, progress_callback=None):
        wp_url = f"https://www.{site}/wp-admin/options-general.php"
        resp = self._request("GET", wp_url)
        if not resp:
            if progress_callback:
                progress_callback(f"无法访问 {wp_url}，使用默认 update_img")
        ...
        if update_img and update_img.get("href"):
            href = update_img.get("href")
            if progress_callback:
                progress_callback(f"从后台提取到 Update Img: {href}")
            return href
        if progress_callback:
            title_tag = soup.find("title")
            page_title = title_tag.text.strip() if title_tag else "(无标题)"
            progress_callback(f"未找到 Update Img 链接，页面标题: {page_title}，使用默认 update_img")
        return "/cf-updata/plxztp.php?p=OFjToUDQ5mmtU7GB"
    ...
    def upload_main_data(self, site, idcode, start_cs="0", progress_callback=None, errortime=0):
        del errortime
        ...
        while True:
            try:
                ...
                update_img = self._get_update_img_url(site, progress_callback)
                base = f"https://www.{site}"
                headers = {"X-Requested-With": "XMLHttpRequest"}
                retrytime = 0

                if progress_callback:
                    progress_callback(f"update_img URL: {update_img}")
                    progress_callback(f"开始上传主数据，lv={idcode}，断点: {cs}")
                ...
                while True:
                    ...
                    if progress_callback:
                        progress_callback(f"请求URL: {upload_url}")
                    try:
                    ...
                    if payload is None:
                        retrytime += 1
                        if progress_callback:
                            progress_callback(f"返回无法解析，继续重试({retrytime})，原始响应: {resp.text[:300]}")
                        time.sleep(2)
                        continue
                    ...
                    if progress_callback:
                        ...
                        full = {k: v for k, v in payload.items() if k != "msg"}
                        if full:
                            progress_callback(f"payload 其他字段: {full}")
